...
impl HtMatchfinder {
    ...
    pub fn longest_match(
        ...
        if in_next + 5 <= input.len() {
            let seq_next = u32::from_le_bytes(input[in_next + 1..in_next + 5].try_into().unwrap());
            *next_hash = lz_hash(seq_next, HT_MATCHFINDER_HASH_ORDER);
        }

        // Load 4 bytes at current position for quick comparison
        let seq = u32::from_le_bytes(input[in_next..in_next + 4].try_into().unwrap());
        ...
        let matchptr_seq = u32::from_le_bytes(input[match_pos..match_pos + 4].try_into().unwrap());
        ...
        if matchptr_seq == seq {
            ...
            let matchptr2_seq =
                u32::from_le_bytes(input[match_pos2..match_pos2 + 4].try_into().unwrap());
