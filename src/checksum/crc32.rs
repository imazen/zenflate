...
//! - VPCLMULQDQ 256-bit folding (x86_64 modern: AVX-512 + VPCLMULQDQ)
...
// ---------------------------------------------------------------------------
// VPCLMULQDQ 256-bit CRC-32 folding (Avx512ModernToken: VPCLMULQDQ + AVX-512)
//
// Processes two 128-bit lanes in parallel per vector. 8 accumulators (8*32=256 bytes
// per main loop iteration). Falls back to 128-bit Barrett reduction at the end.
// ---------------------------------------------------------------------------
...
fn crc32_impl_modern(_token: Avx512ModernToken, crc: u32, data: &[u8]) -> u32 {
    use safe_unaligned_simd::x86_64::{_mm256_loadu_si256, _mm_loadu_si128};

    let len = data.len();

    // Need at least 32 bytes for 256-bit path
    if len < 32 {
        return crc32_slice8(crc, data);
    }

    // 256-bit multiplier vectors: [hi, lo, hi, lo] (duplicated per 128-bit lane)
    ...
    // Use slice advancement instead of index arithmetic to help the compiler
    ...
    // Initialize: load first 32 bytes, XOR CRC into low 32 bits
    let mut x0 = _mm256_xor_si256(
        _mm256_loadu_si256(ld32(p, 0)),
        _mm256_zextsi128_si256(_mm_cvtsi32_si128(crc as i32)),
    );
    p = &p[32..];

    if p.len() >= 224 {
        // 8-accumulator path: need 7 more 32-byte vectors (224 bytes)
        let mut v0 = x0;
        let mut v1 = _mm256_loadu_si256(ld32(p, 0));
        let mut v2 = _mm256_loadu_si256(ld32(p, 32));
        let mut v3 = _mm256_loadu_si256(ld32(p, 64));
        let mut v4 = _mm256_loadu_si256(ld32(p, 96));
        let mut v5 = _mm256_loadu_si256(ld32(p, 128));
        let mut v6 = _mm256_loadu_si256(ld32(p, 160));
        let mut v7 = _mm256_loadu_si256(ld32(p, 192));
        p = &p[224..];

        // Main loop: 256 bytes per iteration (8 vectors × 32 bytes)
        while p.len() >= 256 {
            v0 = fold256!(v0, _mm256_loadu_si256(ld32(p, 0)), mults_8v);
            v1 = fold256!(v1, _mm256_loadu_si256(ld32(p, 32)), mults_8v);
            v2 = fold256!(v2, _mm256_loadu_si256(ld32(p, 64)), mults_8v);
            v3 = fold256!(v3, _mm256_loadu_si256(ld32(p, 96)), mults_8v);
            v4 = fold256!(v4, _mm256_loadu_si256(ld32(p, 128)), mults_8v);
            v5 = fold256!(v5, _mm256_loadu_si256(ld32(p, 160)), mults_8v);
            v6 = fold256!(v6, _mm256_loadu_si256(ld32(p, 192)), mults_8v);
            v7 = fold256!(v7, _mm256_loadu_si256(ld32(p, 224)), mults_8v);
            p = &p[256..];
        }

        // Fold 8 → 4
        v0 = fold256!(v0, v4, mults_4v);
        v1 = fold256!(v1, v5, mults_4v);
        v2 = fold256!(v2, v6, mults_4v);
        v3 = fold256!(v3, v7, mults_4v);
        if p.len() >= 128 {
            v0 = fold256!(v0, _mm256_loadu_si256(ld32(p, 0)), mults_4v);
            v1 = fold256!(v1, _mm256_loadu_si256(ld32(p, 32)), mults_4v);
            v2 = fold256!(v2, _mm256_loadu_si256(ld32(p, 64)), mults_4v);
            v3 = fold256!(v3, _mm256_loadu_si256(ld32(p, 96)), mults_4v);
            p = &p[128..];
        }

        // Fold 4 → 2
        v0 = fold256!(v0, v2, mults_2v);
        v1 = fold256!(v1, v3, mults_2v);
        if p.len() >= 64 {
            v0 = fold256!(v0, _mm256_loadu_si256(ld32(p, 0)), mults_2v);
            v1 = fold256!(v1, _mm256_loadu_si256(ld32(p, 32)), mults_2v);
            p = &p[64..];
        }

        // Fold 2 → 1
        x0 = fold256!(v0, v1, mults_1v);
        if p.len() >= 32 {
            x0 = fold256!(x0, _mm256_loadu_si256(ld32(p, 0)), mults_1v);
            p = &p[32..];
        }
    } else {
        // Small path: fold 32-byte chunks one at a time
        while p.len() >= 32 {
            x0 = fold256!(x0, _mm256_loadu_si256(ld32(p, 0)), mults_1v);
            p = &p[32..];
        }
    ...
    // Reduce 256-bit → 128-bit: fold lo lane into hi lane (128-bit distance)
    let x0_lo = _mm256_castsi256_si128(x0);
    let x0_hi = _mm256_extracti128_si256(x0, 1);
    let mut x128 = fold128!(x0_lo, x0_hi, mults_128b);
...
mod parity {
    ...
    fn avx512_modern_token_available() {
        use archmage::prelude::*;
        let token = Avx512ModernToken::summon();
        assert!(
            token.is_some(),
            "Avx512ModernToken should be available on this CPU"
        );
    }
}
