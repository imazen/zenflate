...
/// SIMD implementations (VPCLMULQDQ 512-bit / PCLMULQDQ / ARM PMULL folding),
/// runtime-dispatched via archmage. Compiled only with the `simd` feature;
/// without it, [`crc32`] uses the scalar slice-by-8 path directly.
#[cfg(feature = "simd")]
mod simd {
    ...
    #[inline]
    pub(super) fn crc32_dispatch(crc: u32, data: &[u8]) -> u32 {
        #[cfg(feature = "avx512")]
        {
    ...
    //
    // These constants (and the fold128/barrett_reduce macros) are only used by
    // the PCLMULQDQ (x86_64) and PMULL (aarch64) SIMD paths. WASM has no
    // carryless-multiply instruction, so CRC-32 uses the scalar slice-by-8
    // fallback there.
    // ---------------------------------------------------------------------------

    // For VL=64 (512-bit): 8v folds across 8*512 = 4096 bits (AVX-512 VPCLMULQDQ only)
    #[cfg(all(target_arch = "x86_64", feature = "avx512"))]
    const CRC32_X4127_MODG: i64 = 0x1072db28_u32 as i64;
    #[cfg(all(target_arch = "x86_64", feature = "avx512"))]
    const CRC32_X4063_MODG: i64 = 0x0c30f51d_u32 as i64;

    // For VL=64 4v / VL=32 8v: folds across 2048 bits (AVX-512 VPCLMULQDQ only)
    #[cfg(all(target_arch = "x86_64", feature = "avx512"))]
    const CRC32_X2079_MODG: i64 = 0xce3371cb_u32 as i64;
    #[cfg(all(target_arch = "x86_64", feature = "avx512"))]
    const CRC32_X2015_MODG: i64 = 0xe95c1271_u32 as i64;

    // x86_64-only: used by both VPCLMULQDQ 512-bit and PCLMULQDQ 128-bit paths
    #[cfg(target_arch = "x86_64")]
    const CRC32_X1055_MODG: i64 = 0x33fff533;
    #[cfg(target_arch = "x86_64")]
    const CRC32_X991_MODG: i64 = 0x910eeec1;

    // Shared across x86_64 and aarch64 SIMD paths
