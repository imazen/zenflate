...
- One-shot convenience free functions for whole-buffer jobs:
  `zenflate::gzip_compress` / `zlib_compress` (take a `CompressionLevel`, return
  a right-sized `Vec`) and `zenflate::gzip_decompress` / `zlib_decompress` (take
  a `max_output_size` hard ceiling that bounds untrusted input — returns
  `DecompressionError::OutputLimitExceeded` rather than allocating past it). Thin
  additive wrappers over `Compressor` / `Decompressor`; require the `alloc`
  feature. README `## Quick start` now leads with them. (free fns 8 → 12)
